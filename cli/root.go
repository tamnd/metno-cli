// Package cli assembles the metno command tree from the metno
...
// NewApp assembles the kit application from the metno domain. The
...
//
// To add a command, declare it in metno/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := metno.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(metno.Domain{}).Register(app)
