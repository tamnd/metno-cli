...
describes what users get. Pick a format with `-o`, or let metno choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
metno <command> -o table   # aligned columns for reading
metno <command> -o jsonl   # one JSON object per line, for piping
metno <command> -o json    # a single JSON array
metno <command> -o csv     # spreadsheet friendly
metno <command> -o tsv     # tab-separated
metno <command> -o url     # just the URL column
metno <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
metno <command> --fields id,title,url
```
...
```bash
metno <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
metno <command>            # a table, because this is a terminal
metno <command> | wc -l    # JSONL, because this is a pipe
```
